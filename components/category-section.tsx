    ...
    <section className={`py-16 ${bgClass}`}>
      <div className="container mx-auto px-4">
        <div className="grid md:grid-cols-2 gap-12 items-center">
          <div>
            <h2 className="text-3xl md:text-4xl font-bold mb-4 text-korean-red">{title}</h2>
            <p className="text-lg text-cream mb-8">{description}</p>

            <div className="space-y-6">
              {laws.map((law) => (
                <div
                  key={law.number}
                  className="bg-korean-red/20 backdrop-blur-sm rounded-lg p-4 border border-korean-red/50 hover:border-korean-red/70 transition-all duration-300 hover:shadow-lg"
                >
                  <div className="flex items-start gap-3">
                    <div className="h-8 w-8 flex items-center justify-center rounded-full bg-korean-red/80 text-cream font-bold flex-shrink-0 mt-1">
                      {law.number}
                    </div>
                    <div>
                      {(() => {
                        ...
                        return (
                          <>
                            ...
                            <p className="text-cream/80 line-clamp-2">{content.substring(0, 120)}...</p>
                          </>
                        )
                      })()}
                      <a
                        href={`#law-${law.number}`}
                        className="text-korean-red hover:text-cream/80 text-sm font-medium mt-2 inline-block"
                      >
                        {t('readMore')}
                      </a>
                    </div>
          ...
          <div className="relative h-80 md:h-[500px] rounded-xl overflow-hidden shadow-2xl">
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-river-depths/80 to-transparent"></div>
          </div>
        </div>
      </div>
    </section>
