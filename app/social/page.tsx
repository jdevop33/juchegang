    ...
    <>
      ...
      <div className="container mx-auto px-4 py-10">
        ...
        {featuredChannels.length > 0 ? (
          <section className="mb-16">
            <h2 className="text-2xl font-semibold text-[#e8e4df] mb-6">Featured Channels</h2>
        ...
        {/* Trusted Sources */}
        <section className="mb-16">
          <h2 className="text-2xl font-semibold text-[#e8e4df] mb-4">Trusted Sources</h2>
          {/* Featured Channels strip */}
          <div className="flex flex-wrap gap-3 mb-6">
            {youtubeSources.map((src) => (
              <a key={src.id} href={src.url} target="_blank" rel="noopener noreferrer" className="px-3 py-2 rounded-lg bg-[#0d1b2a]/50 border border-[#1b4965] text-[#e8e4df]/90 hover:bg-[#1b4965]/50">
          ...
          {trustedStories.length > 0 ? (
              ...
              {trustedStories.map((item) => (
                <div key={item.id} className="space-y-3">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <a href={item.url} target="_blank" rel="noopener noreferrer" className="text-[#e8e4df]/70 underline">
                      Original source
                    </a>
                  </div>
                  {item.youtubeUrl ? (
            ...
            <p className="text-[#e8e4df]/60 mb-10">No trusted stories available right now.</p>
          )}
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {trustedVideos.map((v) => (
                <div key={v.id} className="space-y-2">
                  ...
                  <div className="text-[#e8e4df]/80 text-sm">{v.sourceName ? `${v.sourceName} • ` : ""}{new Date(v.publishedAt).toLocaleString()}</div>
                  <a href={v.url} target="_blank" rel="noopener noreferrer" className="text-[#e8e4df] font-medium hover:underline">{v.title}</a>
                </div>
              ))}
            </div>
          ) : null}
          <p className="text-xs text-[#e8e4df]/50 mt-4">
        ...
        <section className="mb-16">
          <h2 className="text-2xl font-semibold text-[#e8e4df] mb-4">Profiles</h2>
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4">
            {socialProfiles.map((p) => (
              ...
              >
                ...
                <span className="inline-flex items-center justify-center h-9 w-9 rounded-full bg-[#1b4965]/50 text-[#e8e4df] capitalize">
                  {p.platform[0]}
                </span>
                <div>
                  <div className="text-[#e8e4df] font-medium">{p.displayName ?? `@${p.handle}`}</div>
                  <div className="text-[#e8e4df]/60 text-sm">{p.platform}</div>
                </div>
              </a>
            ))}
          </div>
        ...
        <section className="mb-12">
          <h2 className="text-2xl font-semibold text-[#e8e4df] mb-4">Instagram</h2>
          <div className="flex flex-wrap gap-4 mb-6">
            <a
              href="https://instagram.com/jpandajames39"
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center px-4 py-2 bg-gradient-to-r from-[#c73032] to-[#d4a74a] text-[#e8e4df] rounded-lg hover:from-[#a82729] hover:to-[#b8923f] transition-colors"
            >
          ...
          {ig.length === 0 ? (
            <p className="text-[#e8e4df]/60">Instagram feed integration coming soon. Follow us on Instagram for the latest updates!</p>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {ig.map((p) => (
                <a key={p.id} href={p.permalink} target="_blank" className="block rounded-lg overflow-hidden border border-[#1b4965] bg-[#0d1b2a]/50 hover:bg-[#1b4965]/40">
                  <img src={p.mediaUrl} alt={p.caption || "Instagram post"} className="w-full aspect-square object-cover" />
                  <div className="p-4">
                    <p className="text-[#e8e4df]/80 text-sm line-clamp-2">{p.caption}</p>
                    <p className="text-[#e8e4df]/50 text-xs mt-1">{new Date(p.timestamp).toLocaleString()}</p>
                  </div>
                </a>
              ))}
            </div>
        ...
        <section className="mb-12">
          <h2 className="text-2xl font-semibold text-[#e8e4df] mb-4">YouTube</h2>
          {yt.length === 0 ? (
            <p className="text-[#e8e4df]/60">YouTube channel coming soon. Stay tuned for video content!</p>
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {yt.map((v) => (
                <a key={v.id} href={v.url} target="_blank" className="block rounded-lg overflow-hidden border border-[#1b4965] bg-[#0d1b2a]/50 hover:bg-[#1b4965]/40">
                  ...
                  <div className="p-4">
                    <h3 className="text-[#e8e4df] font-medium line-clamp-2">{v.title}</h3>
                    <p className="text-[#e8e4df]/50 text-sm mt-1">{new Date(v.publishedAt).toLocaleString()}</p>
                  </div>
                </a>
              ))}
            </div>
        ...
        <section className="mb-12">
          <h2 className="text-2xl font-semibold text-[#e8e4df] mb-4">Audio</h2>
          ...
          <p className="mt-2 text-xs text-[#e8e4df]/60">
        ...
        <section>
          <h2 className="text-2xl font-semibold text-[#e8e4df] mb-4">Contact</h2>
          <div className="bg-[#0d1b2a]/50 border border-[#1b4965] rounded-lg p-6">
            <p className="text-[#e8e4df] mb-2">For inquiries and collaborations:</p>
            <a
