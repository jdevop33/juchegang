  ...
  return (
    <article className="container mx-auto px-4 pt-28 pb-20 prose prose-invert max-w-4xl">
      <header className="not-prose mb-8">
        <h1 className="text-4xl md:text-5xl font-bold text-cream">{t('sanctionsWarfareTitle')}</h1>
        <p className="text-cream-muted mt-2">{t('sanctionsWarfareDesc')}</p>
      </header>
      ...
      <footer className="not-prose mt-12 text-sm text-cream-muted">
        <div className="flex items-center justify-between gap-4 flex-wrap">
          <p className="text-cream-muted">{t('briefingNote')}</p>
          <LinkedInShare />
        </div>
      </footer>
    </article>
